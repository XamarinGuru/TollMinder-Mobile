...
let RestoreSubmodulePackages dirPath = 
    Directory.EnumerateFiles (dirPath, "*.sln", SearchOption.AllDirectories) 
    |> fun solutions -> 
        for solution in solutions do 
            let packagesPath = Path.Combine(DirectoryInfo(solution).Parent.FullName, "packages")
            Exec "tools/NuGet/NuGet.exe" ("restore " + solution + " -PackagesDirectory " + packagesPath)     
...
let RunUITests appPath =
    ...
    RestorePackages "Tests/GoClientApp.UITests/GoClientApp.UITests.sln"

    MSBuild "Tests/GoClientApp.UITests/bin/Debug" "Build" [ ("Configuration", "Debug"); ("Platform", "Any CPU") ] [ "Tests/GoClientApp.UITests/GoClientApp.UITests.sln" ] |> ignore

    RunNUnitTests "Tests/GoClientApp.UITests/bin/Debug/GoClientApp.UITests.dll" "Tests/GoClientApp.UITests/bin/Debug/testresults.xml"
